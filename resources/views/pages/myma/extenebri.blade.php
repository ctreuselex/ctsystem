@extends('templates.single')

@section('metas')
	<script type="text/javascript">
		var heroImage = 'https://cdna.artstation.com/p/assets/images/images/011/581/178/large/marta-nael-owain-final.jpg?1530302327';
		var mdColor = '#B027AE';
		var sdColor = '#3797D5';
	</script>
@stop

@section('title') Extenebri @stop
@section('tag') "From the Void, bring it Forth" @stop
@section('cont') 
	<p class="head">ORIGIN</p>
	<p>It was first discovered by Frederick Lemaitre in the early 1600s during the rise of the Dominion. Its first use is with Lemaitre's development of the CTsystem, the creation of technology being able to replicate the art of Mystics themselves, this in itself became a separate art form known as: Tekinesi.</p>
	<p class="head">PRACTICE</p>
	<p>It is the art of manipulating Myst all by itself, whether be in it's gaseous-like form or a more practical, crystalized form. Extenebri rapidly climbed on the ranks of Mystical arts since its discovery due to how much easier it is compare to other practices. Mystics, who are not able to identify their material affinity was able to quickly progress in the art which allowed even Mydows to discover their potential in Myst manipulations.</p>
	<p>Dominating the field, comes the manufacturers; who uses the endless posibility of Extenebri to create technology and encouraged the participation of Mystics and Mydows into joining the Institute and serve the city despite of their shortcomings.</p>
	<p>
		<myma class="showname showdes">detection</myma>
		<myma class="showname showdes">crystalization</myma>
		<myma class="showname showdes">remotion</myma>
	</p>
	
    <script src="{{ url('js/myma.js') }}"></script>
@stop